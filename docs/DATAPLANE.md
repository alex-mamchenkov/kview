...
**Namespaced snapshot kinds:** pods, deployments, daemonsets, statefulsets, replicasets, jobs, cronjobs, services, ingresses, persistentvolumeclaims, configmaps, secrets, serviceaccounts, roles.
...
- **Deferred namespaced lists** (rolebindings, helmreleases) and **`GET /api/helmcharts`** remain direct reads until snapshot semantics exist.
