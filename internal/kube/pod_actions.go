...
func validatePodTarget(req ActionRequest) error {
	if req.Group != "" {
		return fmt.Errorf("unsupported group %q, expected \"\"", req.Group)
	}
	if req.Resource != "pods" {
		return fmt.Errorf("unsupported resource %q, expected \"pods\"", req.Resource)
	}
	if req.Namespace == "" {
...
func HandlePodDelete(ctx context.Context, c *cluster.Clients, req ActionRequest) (*ActionResult, error) {
	if err := validatePodTarget(req); err != nil {
	...
	err := c.Clientset.CoreV1().Pods(req.Namespace).Delete(ctx, req.Name, opts)
	if err != nil {
		return nil, err
	}
	...
	return &ActionResult{
		Status:  "ok",
		Message: fmt.Sprintf("Deleted pod %s/%s", req.Namespace, req.Name),
