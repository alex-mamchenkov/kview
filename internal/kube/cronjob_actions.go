...
func validateCronJobTarget(req ActionRequest) error {
	if req.Group != "batch" {
		return fmt.Errorf("unsupported group %q, expected \"batch\"", req.Group)
	}
	if req.Resource != "cronjobs" {
		return fmt.Errorf("unsupported resource %q, expected \"cronjobs\"", req.Resource)
	}
	if req.Namespace == "" {
...
func HandleCronJobDelete(ctx context.Context, c *cluster.Clients, req ActionRequest) (*ActionResult, error) {
	if err := validateCronJobTarget(req); err != nil {
	...
	err := c.Clientset.BatchV1().CronJobs(req.Namespace).Delete(ctx, req.Name, opts)
	if err != nil {
		return nil, err
	}
	...
	return &ActionResult{
		Status:  "ok",
		Message: fmt.Sprintf("Deleted cronjob %s/%s", req.Namespace, req.Name),
