...
func validateServiceTarget(req ActionRequest) error {
	if req.Group != "" {
		return fmt.Errorf("unsupported group %q, expected \"\"", req.Group)
	}
	if req.Resource != "services" {
		return fmt.Errorf("unsupported resource %q, expected \"services\"", req.Resource)
	}
	if req.Namespace == "" {
...
func HandleServiceDelete(ctx context.Context, c *cluster.Clients, req ActionRequest) (*ActionResult, error) {
	if err := validateServiceTarget(req); err != nil {
	...
	err := c.Clientset.CoreV1().Services(req.Namespace).Delete(ctx, req.Name, opts)
	if err != nil {
		return nil, err
	}
	...
	return &ActionResult{
		Status:  "ok",
		Message: fmt.Sprintf("Deleted service %s/%s", req.Namespace, req.Name),
