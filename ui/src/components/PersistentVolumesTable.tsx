...
function PersistentVolumesToolbar(props: {
  filter: string;
  ...
  return (
    <GridToolbarContainer sx={{ display: "flex", flexDirection: "column", gap: 1, p: 1 }}>
      <Box sx={{ display: "flex", gap: 1, alignItems: "center", flexWrap: "wrap" }}>
        <TextField
          size="small"
          label="Filter (name/phase/storageClass/claim)"
          value={props.filter}
...
export default function PersistentVolumesTable({ token }: { token: string }) {
  ...
  const filteredRows = useMemo(() => {
    ...
    return rows.filter((r) => {
      return (
        ...
        (r.storageClassName || "").toLowerCase().includes(q) ||
        (r.reclaimPolicy || "").toLowerCase().includes(q) ||
  ...
  const ToolbarAny = PersistentVolumesToolbar as any;
