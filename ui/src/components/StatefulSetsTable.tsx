...
function StatefulSetsToolbar(props: {
  filter: string;
...
export default function StatefulSetsTable({ token, namespace }: { token: string; namespace: string }) {
  ...
  const filteredRows = useMemo(() => {
    ...
    return rows.filter((r) => {
      return r.name.toLowerCase().includes(q) || (r.serviceName || "").toLowerCase().includes(q);
    });
  }, [rows, filter]);
  ...
  const ToolbarAny = StatefulSetsToolbar as any;
