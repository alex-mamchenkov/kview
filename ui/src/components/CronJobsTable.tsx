...
function CronJobsToolbar(props: {
  filter: string;
...
export default function CronJobsTable({ token, namespace }: { token: string; namespace: string }) {
  ...
  const filteredRows = useMemo(() => {
    ...
    return rows.filter((r) => {
      return r.name.toLowerCase().includes(q) || (r.schedule || "").toLowerCase().includes(q);
    });
  }, [rows, filter]);
  ...
  const ToolbarAny = CronJobsToolbar as any;
